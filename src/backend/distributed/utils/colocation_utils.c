...
{
	Oid destinationRelationId = PG_GETARG_OID(0);
	text *colocateWithTableNameText = PG_GETARG_TEXT_P(1);

	CheckCitusVersion(ERROR);
	EnsureCoordinator();
	EnsureTableOwner(destinationRelationId);
	EnsureDistributedTable(destinationRelationId,
						   "can update colocation of only a distributed table.");

	char *colocateWithTableName = text_to_cstring(colocateWithTableNameText);
	if (IsColocateWithNone(colocateWithTableName))
	{
		BreakColocation(destinationRelationId);
	}
	else
	{
		Oid sourceRelationId = ResolveRelationId(colocateWithTableNameText, false);
		MarkTablesColocated(sourceRelationId, destinationRelationId);
	}
	PG_RETURN_VOID();
}
